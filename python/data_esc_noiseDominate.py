...
S3_PREFIX = "tfrecords"
if DEBUG:
    SNR_DBS_TARGET_SPEECH = [ 6]
    SNR_DBS_TARGET_NOISE = [30]
else:
    SNR_DBS_TARGET_SPEECH = [3, 6, 9, 12, 15, 30]
    SNR_DBS_TARGET_NOISE  = [6, 9, 12, 15, 30]
NTYPES = [
    'ESC-50-MASTER',
    'wham_noise',
    'FSD50K',
        ...
        for i in range(len(self.wavs_noise) >> 1):
            ...
            for k in range(2):
                ...
                try:
                    ...
                    # <---stime---><---speech--->etime<---size_zeros--->|
                    etime = len(speech_raw) + stime

                    # concat front and end to target
                    ...
                    random.shuffle(SNR_DBS_TARGET_NOISE)
                    speech0, _ = add_noise.add_noise(
                                speech0,
                                noise_filler,
                                SNR_DBS_TARGET_NOISE[0],
                    ...
                    etime += FRAMES_TARGET_DELAY * self.feat_inst.hop
                    ...
                    if np.random.uniform(0, 1) < 0.5:
                        ...
                        speech_filler = self.audio_load(
                            ...
                            self.feat_inst.sample_rate)
                        random.shuffle(SNR_DBS_TARGET_SPEECH)
                        speech_filler = self.audio_set_len(speech_filler, len(speech0))
                        speech0, _ = add_noise.add_noise(
                                speech0,
                                speech_filler,
                                SNR_DBS_TARGET_SPEECH[0],
                                stime=0, etime=len(speech0),
                                return_all=True,
                                snr_dB_improved = None,
                                rir=None,
