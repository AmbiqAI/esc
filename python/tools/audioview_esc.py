...
class DataServiceClass:
    ...
    def __init__(
            ...
            esc_class,
            update_esc_class):
        self.update_esc_class = update_esc_class
    ...
    def ns_rpc_data_sendBlockToPC(self, pcmBlock): # pylint: disable=invalid-name
        ...
        if is_record == 0:
            ...
            if (pcmBlock.cmd == GenericDataOperations_EvbToPc.common.command.write_cmd) \
                ...
                update_esc_class = self.update_esc_class[0]
                self.lock.release()
                
                if update_esc_class == 1:
                    self.esc_class[0] = esc_class
                    if esc_class != 0:
                        self.lock.acquire()
                        self.update_esc_class[0] = 0
                        self.lock.release()
...
class VisualDataClass:
    ...
    def __init__(
        ...
        self.ax_handle[0].title.set_text("Sound Classification Viewer")
        ...
        self.ax_handle[1].text(-0.1,0,"Trigger", ha='right')
        ...
        self.text_info = plt.text(1.2,0.4, "Now:")
        self.text_info_last = plt.text(1.2,-0.1, "Last:")

        self.lock.acquire()
        esc_class  = self.esc_class[0]
        self.lock.release()
        self.text_info.set_text(f"Now: {CLASSSES_SET[esc_class]}")
        self.esc_class_last = 0
    ...
    def callback_recordstart(self, event):
        ...
        if is_record == 0:
            ...
            while 1:
                ...
                esc_class  = self.esc_class[0]
                self.update_esc_class[0] = 1
                self.lock.release()

                if self.esc_class_last != 0:
                    self.text_info_last.set_text(f"Last: {CLASSSES_SET[self.esc_class_last]}")

                if esc_class != 0:
                    self.esc_class_last = esc_class
