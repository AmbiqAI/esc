        ...
        PARAMS_NNSP *pt_params)
{
    ...
    pt_inst->pt_state_nnid = (pt_inst->nn_id== nnid_id) ? 
            (void*) &state_nnid : (void*) 0;
        ...
        int16_t* rawPCM)
{
    ...
    int16_t* pt_inputs;
    // for (int i = 0; i < pt_inst->pt_params->hopsize_stft; i++)
    // {
    //     rawPCM[i] = (rawPCM[i] * (int16_t) pt_inst->pt_params->pre_gain_q1) >> 1; // Q1
    // }
...
{
    int16_t idx_max;
    int16_t th_count_trigger = *pt_inst->pt_th_count_trigger;
    int i;
    my_argmax(pt_est, 5, &idx_max);
    if (pt_inst->counts_category[idx_max] == 0)
    {
        for (i = 0; i < 5; i++)
            pt_inst->counts_category[i] = 0;
    }
    if ( pt_inst->counts_category[idx_max] < th_count_trigger)
        pt_inst->counts_category[idx_max]+=1;
    *pt_trigger = (pt_inst->counts_category[idx_max] >= th_count_trigger) ? idx_max : 0;
}
